...
Add the migration hook, because `CREATE TABLE IF NOT EXISTS` cannot widen a
constraint on a table that already exists. Above `connect()`:
...
and call it in `connect()`, **before** `executescript(SCHEMA)`:
...
> **Note.** Expanding `MANAGER_DEFAULTS` from three keys to seven breaks the
> pre-existing `test_missing_file_is_defaults`, which asserts an exact literal
> dict. **Extend that literal to seven keys — do not repoint the assertion at
> `config.MANAGER_DEFAULTS`.** Comparing a function's output to the very
> constant it returns is tautological: it passes against a `read_manager_cfg`
> that does nothing but `return dict(MANAGER_DEFAULTS)`, and it leaves the
> shipped values of `db_path` and `poll_slow` pinned by nothing at all.
...
> **Note on the `overall` row.** It stores the three-value chip
> (`ok|degraded|unknown`) directly in `state` — the column's CHECK admits both
> vocabularies, because the table holds two kinds of row. `basis` keeps its
> meaning everywhere: for an indicator it says why that indicator reads as it
> does, and for the overall row it names the indicators or blind domains that
> dragged the node down, so the pane can say "Degraded — capacity, thermal".

- [ ] **Step 5: Call it from `run_cycle`**

In `run_cycle`, inside the existing `if lane == collector.FAST:` block, as the
**last** statement — after `scheduler.record(...)` and after the
`poll_fail`/`poll_ok` journal handling:

```python
            self._update_health(node_id, results, stamp)
```

**Order is load-bearing.** `_safe_cycle` swallows and logs any exception from a
cycle, so anything raised by health evaluation would skip whatever follows it.
Put it first and one bad evaluation silently disables backoff, the unknown
threshold and the event journal for that node — the daemon would keep hammering
an unreachable box every 30s with nothing in the log to say why. Health is the
least critical of the three; it goes last.
